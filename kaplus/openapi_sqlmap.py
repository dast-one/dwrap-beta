...
def _sample_for_schema(sd):
    ...
    if sd['type'] == 'object':
        return dict((p, _sample_for_schema(s)) for (p, s) in sd.get('properties', dict()).items())
    elif sd['type'] == 'array':
        return list(_sample_for_schema(sd['items']) for _ in range(3))
    else:
        return {
            'string': 'qwe',
            'integer': 123,
